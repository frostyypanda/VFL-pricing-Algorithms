"""Generate GW4 transfer plan for T1 and T2 using W1+W2+W3 actuals.

Approach:
  1. Load full historical data + calibrated EP model.
  2. Load W1, W2, W3 VLR results (w*_vlr_results.json).
  3. Blend all three weeks into EP (Bayesian update, stronger weight with more samples).
  4. For each user team (T1, T2), run slot-locked ILP GW4 -> GW6 using ManualVP prices.
  5. Constraint: reach 11 AMER players by GW6 (max 3 transfers/week, 3 weeks).
  6. Emit a markdown report per team.
"""
import sys
import os
import io
...
TEAM_1 = [
    ("D1", "Eggsterr"), ("D2", "Primmie"),
    ("C1", "Veqaj"),    ("C2", "Patmen"),
    ("I1", "Killua"),   ("I2", "Rosé"),
    ("S1", "Free1ng"),  ("S2", "Yetujey"),
    ("W1", "Jemkin"),   ("W2", "Udotan"), ("W3", "Saadhak"),
]
...
def blend_actuals(ep, weeks):
    ...
    weeks: list of dicts {player: [match_entries]}.
    """
    df = ep.copy()
    alpha_per_week = 0.25  # each match pulls 25% toward observed
...
def optimize_gw(ep, prices, roster, gw, min_amer):
    ...
    """
    gw_col = f"GW{gw}"
    amer_bonus = {4: 1.0, 5: 2.5, 6: 0.0}.get(gw, 0.0)
...
def run_plan(ep, prices, starting_roster, label):
    ...
    AMER target schedule for GW4-6: hit 11 by GW6 with 3 transfers/week.
    """
...
def render_team_plan(label, start_spec, plan, ep, prices):
    lines = [f"# {label} — GW4-6 Transfer Plan\n"]
    lines.append("## Starting Roster (post-GW3)\n")
...
def main():
    ...
    print("[2/5] Loading W1 + W2 + W3 VLR actuals...")
    ...
    w3 = load_week_results(os.path.join(DIR, "data", "w3_vlr_results.json"), our_players)
    print(f"  W1 matched {len(w1)} players, W2 matched {len(w2)} players, W3 matched {len(w3)} players")

    print("[3/5] Blending actuals into EP...")
    ep_updated = blend_actuals(ep, [w1, w2, w3])
    ...
    for label, spec in [("T1", TEAM_1), ("T2", TEAM_2)]:
        ...
        out_path = os.path.join(DIR, "output", f"GW4_transfer_plan_{label}.md")
